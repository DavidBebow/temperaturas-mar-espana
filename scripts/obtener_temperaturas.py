...
def obtener_temperatura_actual(lat, lon):
    ...
    try:
        ...
        if temps_validas:
            return round(temps_validas[-1], 1)
        return None
...
def calcular_anomalia(temp_actual, punto_id):
    mes_actual = datetime.now().month - 1
    if punto_id in MEDIAS_HISTORICAS and temp_actual is not None:
...
def calcular_max_racha(entradas, signo):
    """Calcula la racha máxima de días consecutivos con un signo dado."""
    max_racha = 0
    racha_actual = 0
...
def actualizar_historial(resultados):
    ruta_historial = "docs/historial_temperaturas.json"
    ...
    for punto in resultados:
        ...
        signo_hoy = "positiva" if anomalia > 0 else "negativa" if anomalia < 0 else "normal"
        ...
        # Mantener solo los últimos 365 días ordenados de más reciente a más antiguo
...
def generar_json():
    ...
    for punto in PUNTOS:
        ...
        if temp_actual is not None:
            anomalia, media_historica = calcular_anomalia(temp_actual, punto["id"])
        ...
        if temp_actual is not None:
            print(f"  ✓ {temp_actual}°C | Anomalía: {'+' if anomalia and anomalia > 0 else ''}{anomalia}°C")
        else:
            print(f"  ✗ Sin datos")
    ...
    with open("docs/datos.json", "w", encoding="utf-8") as f:
        json.dump(output, f, ensure_ascii=False, indent=2)

    print(f"\n✓ JSON guardado en docs/datos.json")
