...
# ─────────────────────────────────────────────────────────────────────────────
# TABLA DE CORRESPONDENCIA: código INE provincia → prefijos de indicativo AEMET
# Los indicativos de estación AEMET NO coinciden con el código INE.
# Esta tabla se construyó cruzando el inventario de estaciones de AEMET con
# los códigos INE. Algunas provincias tienen varios prefijos.
# ─────────────────────────────────────────────────────────────────────────────
INE_A_PREFIJOS_AEMET = {
    "01": ["01"],           # Álava
    "02": ["7"],            # Albacete
    "03": ["8"],            # Alicante
    "04": ["6"],            # Almería
    ...
    "44": ["8"],            # Teruel
    "45": ["3"],            # Toledo
    "46": ["8"],            # Valencia  ← prefijo AEMET es 8, no 46
    "47": ["2"],            # Valladolid
    "48": ["1"],            # Bizkaia
    "49": ["1"],            # Zamora
    "50": ["9"],            # Zaragoza
...
PROVINCIAS = [
    ...
    {"id": "canarias_sc_tf", "nombre": "S.C. Tenerife",            "ccaa": "Canarias",            "id_ine": "38", "lat": 28.46, "lon": -16.25, "media_anual_mm": 300},
...
def aemet_get(endpoint):
    ...
    try:
        ...
        if meta.get("estado") != 200:
            print(f"  ✗ AEMET estado {meta.get('estado')} [{endpoint[:60]}]")
            return None
        r2 = requests.get(meta["datos"], timeout=30)
        return r2.json()
    except Exception as e:
        print(f"  ✗ Error AEMET [{endpoint[:60]}]: {e}")
        return None

def fecha_aemet(d):
    return d.strftime("%Y-%m-%dT00:00:00UTC")

def obtener_precipitacion_periodo(fecha_ini, fecha_fin, acumular=False):
    """
    Descarga observaciones climatológicas diarias para un período.
    acumular=True → suma todos los días (para semana/anual)
    acumular=False → media de un solo día (para ayer)
    Devuelve {id_ine: mm}
    """
    ...
    # Construimos tabla inversa: prefijo_aemet → id_ine
    prefijo_a_ine = {}
    for id_ine, prefijos in INE_A_PREFIJOS_AEMET.items():
        for p in prefijos:
            prefijo_a_ine[p] = id_ine

    def indicativo_a_ine(indicativo):
        # Intentamos prefijos de 2 chars primero, luego 1 char
        for longitud in (2, 1):
            prefijo = indicativo[:longitud]
            if prefijo in prefijo_a_ine:
                return prefijo_a_ine[prefijo]
        return None

    # {id_ine: {estacion: [mm por día]}}
    por_estacion = {}
    for obs in datos:
        indicativo = obs.get("indicativo", "")
        id_ine = indicativo_a_ine(indicativo)
        if not id_ine:
            continue
        try:
            prec_str = str(obs.get("prec", "0")).replace(",", ".").strip()
            if prec_str in ("", "Ip", "Varias", "None"):
                prec_mm = 0.0
            else:
        ...
        if id_ine not in por_estacion:
            por_estacion[id_ine] = {}
        if indicativo not in por_estacion[id_ine]:
            por_estacion[id_ine][indicativo] = 0.0
        por_estacion[id_ine][indicativo] += prec_mm  # suma días por estación

    resultado = {}
    for id_ine, estaciones in por_estacion.items():
        valores = list(estaciones.values())
        if valores:
            # Media entre estaciones de la provincia (cada una ya tiene su acumulado)
            resultado[id_ine] = round(sum(valores) / len(valores), 1)
    return resultado

def calcular_color(mm):
...
def calcular_color_anomalia(anomalia):
...
def procesar_lluvias():
    ...
    print(f"{'✓ API AEMET activa' if usar_api else '⚠ Sin API key'}")

    if usar_api:
        print("  → Descargando ayer...")
        prec_ayer   = obtener_precipitacion_periodo(ayer, ayer, acumular=False)
        print(f"     {len(prec_ayer)} provincias con datos")

        print("  → Descargando últimos 7 días...")
        prec_semana = obtener_precipitacion_periodo(inicio_semana, ayer, acumular=True)
        print(f"     {len(prec_semana)} provincias con datos")

        print("  → Descargando acumulado anual...")
        prec_anual  = obtener_precipitacion_periodo(datetime(hoy.year, 1, 1), ayer, acumular=True)
        print(f"     {len(prec_anual)} provincias con datos")
    else:
        prec_ayer = prec_semana = prec_anual = {}
    ...
    for prov in PROVINCIAS:
        ...
        if usar_api and prec_ayer:
            mm_ayer   = prec_ayer.get(id_ine, 0.0)
            mm_semana = prec_semana.get(id_ine, 0.0)
            mm_anual  = prec_anual.get(id_ine, 0.0)
        else:
            # Fallback solo si no hay API
            import random
            norte = prov["ccaa"] in ("Galicia","Asturias","Cantabria","País Vasco","Navarra")
            mm_ayer   = round(random.uniform(0, 18 if norte else 4), 1)
            mm_semana = round(mm_ayer * random.uniform(2.5, 5.0), 1)
            fraccion  = dia_del_anio / 365
            mm_anual  = round(prov["media_anual_mm"] * fraccion * random.uniform(0.7, 1.3), 0)
        ...
        provincias_resultado.append({
            ...
            "color_ayer":        calcular_color(mm_ayer),
            "color_semana":      calcular_color(mm_semana),
            "color_anual":       calcular_color(mm_anual),
            "color_anom_ayer":   calcular_color_anomalia(anom_ayer),
            "color_anom_semana": calcular_color_anomalia(anom_semana),
            "color_anom_anual":  calcular_color_anomalia(anom_anual),
        })
    ...
    output = {
        ...
        "fecha_semana":   (hoy - timedelta(days=7)).strftime("%d/%m/%Y") + " – " + ayer.strftime("%d/%m/%Y"),
        "anio":           str(hoy.year),
